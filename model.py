from __future__ import annotations

from pathlib import Path

import torch
from peft import LoraConfig, get_peft_model
from speakerlab.process.processor import FBank
from speakerlab.utils.builder import build
from speakerlab.utils.config import build_config
from speakerlab.utils.utils import load_params
from torch import nn
from transformers import AutoModel

def _mean_pool(last_hidden_state: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    mask = attention_mask.unsqueeze(-1).to(last_hidden_state.dtype)
    return (last_hidden_state * mask).sum(dim=1) / mask.sum(dim=1).clamp_min(1e-9)


def _linear_leaf_names(module: nn.Module) -> list[str]:
    return sorted({name.rsplit(".", 1)[-1] for name, child in module.named_modules()
                   if isinstance(child, nn.Linear)})


class BertRdinoModel(nn.Module):
    def __init__(
        self,
        *,
        text_model_name: str,
        rdino_yaml: str | Path,
        rdino_checkpoint: str | Path,
        audio_embedding_dim: int = 512,
        fusion_dim: int = 128,
        num_classes: int = 2,
        dropout: float = 0.1,
        lora_rank: int = 2,
        lora_alpha: int = 16,
        sample_rate: int = 16_000,
    ) -> None:
        super().__init__()
        text_model = AutoModel.from_pretrained(text_model_name, use_safetensors=True)
        self.text_embedding_dim = text_model.config.hidden_size
        self.text_model = get_peft_model(
            text_model,
            LoraConfig(
                r=lora_rank,
                lora_alpha=lora_alpha,
                target_modules=["query", "value"],
                lora_dropout=dropout,
                bias="none",
            ),
        )

        rdino_yaml = Path(rdino_yaml).expanduser().resolve()
        rdino_checkpoint = Path(rdino_checkpoint).expanduser().resolve()
        config = build_config(str(rdino_yaml))
        teacher = build("teacher_model", config)
        checkpoint = torch.load(rdino_checkpoint, map_location="cpu", weights_only=False)
        if "teacher" not in checkpoint:
            raise KeyError(f"RDINO checkpoint has no 'teacher' state: {rdino_checkpoint}")
        self.rdino_model = load_params(teacher, checkpoint["teacher"])

        targets = _linear_leaf_names(self.rdino_model.backbone)
        if not targets:
            raise RuntimeError("No linear layers found in the RDINO backbone for LoRA")
        self.rdino_model.backbone = get_peft_model(
            self.rdino_model.backbone,
            LoraConfig(
                r=lora_rank,
                lora_alpha=lora_alpha,
                target_modules=targets,
                lora_dropout=dropout,
                bias="none",
            ),
        )

        # Match 3D-Speaker's pretrained RDINO input: 80-D Kaldi-style fbank
        # features with per-recording mean normalization.
        self.feature_extractor = FBank(
            n_mels=80, sample_rate=sample_rate, mean_nor=True
        )
        self.text_projection = nn.Sequential(
            nn.Linear(self.text_embedding_dim, fusion_dim), nn.SiLU(), nn.Dropout(dropout)
        )
        self.audio_projection = nn.Sequential(
            nn.Linear(audio_embedding_dim, fusion_dim), nn.SiLU(), nn.Dropout(dropout)
        )
        self.fusion = nn.Sequential(
            nn.BatchNorm1d(fusion_dim * 2),
            nn.Linear(fusion_dim * 2, fusion_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
        )
        self.classifier = nn.Linear(fusion_dim, num_classes)
        self.regressor = nn.Linear(fusion_dim, 1)
        self._configure_trainable_parameters()

    def _configure_trainable_parameters(self) -> None:
        for parameter in self.parameters():
            parameter.requires_grad = False
        for name, parameter in self.named_parameters():
            if "lora_" in name:
                parameter.requires_grad = True
        for module in (
            self.text_projection,
            self.audio_projection,
            self.fusion,
            self.classifier,
            self.regressor,
        ):
            for parameter in module.parameters():
                parameter.requires_grad = True

    def forward(self, tokens: dict[str, torch.Tensor], waveforms: torch.Tensor):
        text_output = self.text_model(**tokens)
        text_embedding = _mean_pool(text_output.last_hidden_state, tokens["attention_mask"])
        text_embedding = self.text_projection(text_embedding)

        features = torch.stack([self.feature_extractor(waveform) for waveform in waveforms])
        audio_embedding = self.rdino_model.backbone(features)
        audio_embedding = self.audio_projection(audio_embedding)

        fused = self.fusion(torch.cat((text_embedding, audio_embedding), dim=1))
        return self.classifier(fused), self.regressor(fused).squeeze(-1)

    def trainable_parameter_counts(self) -> tuple[int, int]:
        total = sum(parameter.numel() for parameter in self.parameters())
        trainable = sum(
            parameter.numel() for parameter in self.parameters() if parameter.requires_grad
        )
        return trainable, total
